...
function Trigger() {
  const { user, activeOrgPromise } = useAppCtx();
  const activeOrganization = React.use(activeOrgPromise);
    ...
    <>
      ...
      <div className="grid flex-1 text-left text-sm leading-tight">
        <span className="truncate font-medium">
          {activeOrganization ? activeOrganization.name : user.name}
        </span>
      </div>
...
function GroupContent() {
  const { activeOrgPromise, orgListPromise } = useAppCtx();
  const activeOrganization = React.use(activeOrgPromise);
  const organizationList = React.use(orgListPromise);
