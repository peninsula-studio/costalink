 ...
 */
export function extractKyeroProperties(
  xmlData: string,
): z.infer<typeof kyeroPropertySchema>[] {
  ...
  const properties: z.infer<typeof kyeroPropertySchema>[] = root.property.map(
    (rawProp: any) => {
      ...
      let images: z.infer<typeof kyeroImageSchema>[] = [];
      ...
      const extractMultiLang = (
        node: Record<any, any>,
      ): z.infer<typeof i18nStringSchema> => {
        ...
        const typedResult = z.parse(i18nStringSchema, result);
        return typedResult;
      };

      const prop: z.infer<typeof kyeroPropertySchema> = {
        id: rawProp.id,
        date: rawProp.date,
        ref: rawProp.ref,
